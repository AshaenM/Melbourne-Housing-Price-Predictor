...
@app.get("/debug/{bathroom}/{rooms}/{distance}/{bedroom}/{yearbuilt}/{population}/{education}")
async def debug(bathroom: int, rooms: int, distance: int, bedroom: int, yearbuilt: int, population: int, education: int):
    model.load_model()
    scaled = model.scaler.transform([[bathroom, rooms, distance, bedroom, yearbuilt, population, education]])
    raw_prediction = model.model.predict(scaled)
    return {
